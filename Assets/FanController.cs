...
namespace RageRunGames.BowArrowController
{
    ...
    public class FanController : MonoBehaviour
    {
        ...
        [Header("Fan Settings")] public Transform fanTransform;
        ...
        private Vector3 fanOffset;
        private Quaternion fanRotationOffset;
        private bool isFollowingFan = false;
        ...
        private void Start()
        {
            ...
            // 自动判断是否附着风扇（仅第一次进入时调用一次）
            if (fanTransform != null)
            {
                fanOffset = fanTransform.InverseTransformPoint(transform.position);
                fanRotationOffset = Quaternion.Inverse(fanTransform.rotation) * transform.rotation;
                isFollowingFan = true;
            }
        }
        ...
        {
            if (isFollowingFan && fanTransform != null)
            {
                transform.position = fanTransform.TransformPoint(fanOffset);
                transform.rotation = fanTransform.rotation * fanRotationOffset;
            }
        }
        ...
        private void HandleHeadBobbing()
        {
            ...
            if (movementMagnitude > 0.1f)
            {
                ...
                float bobbingOffsetX = Mathf.Cos(timer * 0.5f) * (currentBobAmount * 0.5f);
        ...
        {
            if (bow != null && bow.IsExecuting) return;

            float mouseX = Input.GetAxis("Mouse X") * swayAmount;
            float mouseY = Input.GetAxis("Mouse Y") * swayAmount;

            float movementSwayMultiplier = isSprinting ? 2f : 1f;
            float swayInAir = isGrounded ? 0f : 0.1f;
            ...
            mouseY = Mathf.Clamp(mouseY, -maxSwayAmount, maxSwayAmount) +
                     swayInAir * Mathf.Sign(controller.velocity.y);
